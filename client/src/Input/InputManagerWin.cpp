...
static std::unordered_map<uint16_t, KeyCode> KeyCodeMap = {
    {87, KeyCode::W},      {83, KeyCode::S},
    {65, KeyCode::A},      {68, KeyCode::D},

    {37, KeyCode::LArrow}, {39, KeyCode::RArrow},
    {38, KeyCode::UArrow}, {40, KeyCode::DArrow}};
    ...
    for (int i = 0; i < eventsRead; ++i) {
      if (inputRecords[i].EventType == KEY_EVENT) {
        if (inputRecords[i].Event.KeyEvent.bKeyDown) {
          InputManager::GlobalKeyState
              [KeyCodeMap[inputRecords[i].Event.KeyEvent.wVirtualKeyCode]] =
                  KeyState::Down;
        } else {
          InputManager::GlobalKeyState
              [KeyCodeMap[inputRecords[i].Event.KeyEvent.wVirtualKeyCode]] =
