...
void ME::SceneFallingSand::BuildInstancedSpriteRenderers() {
    ...
    for (size_t i = 0; i < instancedSpriteRendererCount; ++i) {
        ...
        spriteInstanceData[i]->atlasIndex = 3;
        spriteInstanceData[i]->color = ME::Color::Black();
    }
}

void ME::SceneFallingSand::BuildTextRenderers() {
    textRendererCount = 1;
    textTransformsCount = 1;

    // Text 1
    ME::TextRenderer* textRend =
        new ME::TextRenderer{"\x0F FALLING SAND \x0F", 0, 2, 0, ME::Color::White(), 80, 80, -10, 0, 0};
